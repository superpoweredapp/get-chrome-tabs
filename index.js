...
if (process.platform === "darwin") {
	...
	module.exports = async function getChromeTabs(...args) {
		...
		if (argLen === 1) {
			if (!isPlainObj(option)) {
				throw new TypeError(
					`Expected an <Object> to specify get-chrome-tabs option, but got ${inspectWithKind(
						option
					)}.`
				);
			}
			...
			if (option.app !== undefined) {
				if (typeof option.app !== "string") {
					throw new TypeError(
						`${APP_ERROR}, but got a non-string value ${inspectWithKind(
							option.app
						)}.`
					);
				}
				...
				if (!nameIdMap.has(option.app)) {
					throw new RangeError(
						`${APP_ERROR}, but got neither of them ${inspect(option.app)}.`
					);
				}
			}
		} else if (argLen !== 0) {
			throw new RangeError(
				`Expected 0 or 1 argument ([<Object>]), but got ${argLen} arguments.`
			);
		}
	...
	module.exports = async function getChromeTabs() {
		const error = new Error(
			`get-chrome-tabs only supports macOS, but the current platform is ${platformName()}.`
		);
